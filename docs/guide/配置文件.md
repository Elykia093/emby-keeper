...
| `telegram_api_url` | `str` | Telegram Bot API 反代地址, 仅当 `method = "apprise"` 且 `apprise_uri` 使用 `tgram://` 时有效, 未设置则使用官方 API | |
...
# 仅当 apprise_uri 使用 tgram:// 时有效, 未设置则使用官方 Telegram Bot API
telegram_api_url = "https://proxy.elykia.cn/https/api.telegram.org"
```
